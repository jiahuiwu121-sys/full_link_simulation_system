...
namespace DRAMPower
{
	struct command_stats_t : public util::Serialize, public util::Deserialize {
		...
		uint64_t rfmAllBank = 0;
		uint64_t rfmPerBank = 0;
		uint64_t readAuto = 0;
		...
		void serialize(std::ostream& stream) const override {
			...
			stream.write(reinterpret_cast<const char *>(&rfmAllBank), sizeof(rfmAllBank));
			stream.write(reinterpret_cast<const char *>(&rfmPerBank), sizeof(rfmPerBank));
		...
		void deserialize(std::istream& stream) override {
			...
			stream.read(reinterpret_cast<char *>(&rfmAllBank), sizeof(rfmAllBank));
			stream.read(reinterpret_cast<char *>(&rfmPerBank), sizeof(rfmPerBank));
		...
		bool operator==(const command_stats_t& rhs) const {
			return act == rhs.act &&
				...
				rfmAllBank == rhs.rfmAllBank &&
				rfmPerBank == rhs.rfmPerBank &&
	...
	NLOHMANN_JSONIFY_ALL_THINGS(command_stats_t, act, pre, reads, writes, refAllBank, refPerBank, refPerTwoBanks, refDualBanks, refSameBank, rfmAllBank, rfmPerBank, readAuto, writeAuto);
