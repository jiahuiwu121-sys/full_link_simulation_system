...
This workspace extension adds exploratory HBM3 and HBM4 command-level models. See
[the HBM3/HBM4 model documentation](docs/hbm3_hbm4_model.md) for parameter sources,
assumptions, trace format, and runnable examples.
...
An example snippet to initialize an LPDDR4 based DRAM spec can look like this (only the DRAMPower related includes are shown):
...
auto memspec = DRAMUtils::parse_memspec_from_file(
    std::filesystem::path("tests/tests_drampower/resources/lpddr4.json")
);
if (!memspec) {
...
// The next line can throw std::bad_variant_access if the memspec is not an LPDDR4 memspec
// or an exception derived from std::exception if the json object is not valid
LPDDR4 dram(MemSpecLPDDR4::from_memspec(*memspec));
...
$ ./drampower_cli -c config.json -m ../../tests/tests_drampower/resources/lpddr4.json -t ../../tests/tests_drampower/resources/lpddr4.csv
$ ./drampower_cli -c config.json -m ../../tests/tests_drampower/resources/lpddr4.json -t ../../tests/tests_drampower/resources/lpddr4.csv -j output.json
```
