"""Image-aligned HBM3E example: 24 GB, 8-Hi, 8 Gb/s, 1024-bit I/O."""
...
RUNTIME_TICK_PS = 500 // 2  # HBM3E uses two simulator ticks per CK.


def make_controller():
    """Model one independently scheduled 32-bit HBM3E pseudo-channel."""
    # Ramulator2 has no separate HBM3E protocol class.  HBM3E therefore uses
    # the compatible HBM3 command/state model with HBM3E organization/rate
    # presets added specifically for this example.
    dram = ramulator.dram.HBM3(
        org_preset="HBM3E_24Gb_8Hi",
        timing_preset="HBM3E_8000Mbps",
        pseudochannel=1,
    )
